 ...
 * Component를 extends해서 시작하기
 */
class CompBody extends Component {
  // react에서 view용도로 사용하는 변수선언
  // state(상태) 변수 : view용도로 사용하는 변수
  // react는 어딘가에서 state변수의 값이 변동이 되면
    ...
    return (
      <div>
        {/* 
            ...
            h3 tag의 inputText의 표시부분을 rendering 할것이다.
            input box에 클릭후 키보드로 문자열을 입력하면
            input box에서는 onChange 이벤트가 발생할 것이다
            onChange 이벤트를 가로채서(핸들링) inputText의 값을 변화시킨다

            class type에서는 이벤트 핸들러 함수를 쓸때 
            {this.함수명} 이라고 작성해야 한다.
          */}
        <input
        ...
        {/* class type의 컴포넌트에서 state 변수를 화면에 보이기 위해서는
            {this.state.변수병} 형식으로 사용을 해야 한다. */}
