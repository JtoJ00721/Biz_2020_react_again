 ...
 * CompCount에 변화된 갑승ㄹ rendering하여 보여줘야 한다.
 * 같은 컴포너트에서는 그냥 state 변수를 선언하고
 * state 변수를 변화시키는 코드를 작성하면 어렵지 않다.
 * 하지만 지금은 Count와 Button이 같은 형제지간이다
 * react에서는 형제지간에 데이터를 직접 교환하거나
 * 형제의 변수를 변화시키는 행위는 절대 할수 없다.
 * count의 변수를 변화시켜 rendering을 수행하려면
 * Button에서 변화시키는 행위를 부모에게 전달하고
 * 부모 Count 변수를 변화시켜 다시 Count에게 전달해주는
 * 방식을 사용해야 한다.
 *
 ...
 *
 * 5. CompButton count변수를 변환시킬때 사용할 setCount함수를 전달한다
...
function CompBody(props) {
  ...
  return (
    <div>
      ...
      <CompButton setCountFunc={setCount} countArg />
    </div>
  );
}
